...
class AddContactWindow(QtWidgets.QDialog):
    ...
    def add_ctypes(self, vals):
        self.ctype_box.addItems(vals)

    def suggest_relation(self, vals):
        self.suggestions["relation"] = vals
    ...
    def allvals(self, key="all"):
        ...
        if key in ["ctype", "all"]:
            self.contact_model["ctype"] = comboval(self.ctype_box)
        if key in ["who", "all"]:
            self.contact_model["who"] = self.who_edit.text()
        if key in ["relation", "all"]:
            self.contact_model["relation"] = self.relation_edit.text()
        if key in ["cvalue", "all"]:
            self.contact_model["cvalue"] = self.cvalue_edit.text()
