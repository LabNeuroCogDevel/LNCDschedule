...
class AddPersonWindow(QtWidgets.QDialog):
    ...
    def __init__(self, parent=None):
        ...
        # source
        # todo: toggle visitble of text edit if source is not other, use that
        # value
        self.source_text_edit.textChanged.connect(
            lambda: self.allvals('source'))
    ...
    def setpersondata(self, d):
        ...
        for k in self.persondata.keys():
            if k in d:
                self.persondata[k] = d[k]
        if (self.persondata['fname'][:1] == '%'):
            # Get rid of the first % in the name
            self.fname_edit.setText(self.persondata['fname'][1:])
        else:
            # if there is no % sign, simply add everything in it.
