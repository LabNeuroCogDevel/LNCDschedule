...
class MoreInfoWindow(QtWidgets.QDialog):
    ...
    def table_fill(self):
        ...
        if res is None:
            mkmsg(
                "BUG: no db results for %s @ %s! but is still in display?!"
                % (self.vid, self.task)
            )
            return
        ...
        if res[0][0] is None:
            ...
            if isinstance(measures, str):
                mkmsg(
                    "ERROR in data storage. measures should be objects not strings, consider:\n"
                    + "update visit_task set measures = NULL where vid = %s task like '%s'"
                    % (self.vid, self.task)
                )
                print(res[0][0])
                return
    ...
    def add_task(self, measures, vtid):
        self.sql.update(
            "visit_task", "measures", vtid, new_value=measures, id_column="vtid"
        )
