...
def test_gui_reschedule(qtbot, lncdapp):
    ...
    index = lncdapp.visit_table.model().index(0, 1)
    lncdapp.visit_table.setCurrentIndex(index)
    ui_vtype = lncdapp.visit_table.item(0, lncdapp.visit_columns.index("vtype")).text()
    ui_vdate = lncdapp.visit_table.item(0, lncdapp.visit_columns.index("day")).text()
...
def test_gui_reschedule_cal(qtbot, lncdapp):
    ...
    assert lncdapp.visit_table.item(
        0, lncdapp.visit_columns.index("day")
    ...
    old_event = lncdapp.cal.events.list(
        calendarId=lncdapp.cal.backCalID,
        singleEvents=True,
        timeMin=gtime,
        timeMax=gtime,
    ).execute()
    if len(old_event) == 1:
        lncdapp.cal.events().delete(
            eventId=old_event[0]["id"], calendarId=lncdapp.cal.calendarId
        ).execute()
