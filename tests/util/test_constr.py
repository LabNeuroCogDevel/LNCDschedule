...
def test_make_connstr():
    ...
    assert make_connstr(defdb) == "dbname=db user=uname host=h"
    assert (
        make_connstr({**defdb, "password": "pwd"})
        == "dbname=db user=uname host=h password=pwd"
    )
    assert (
        make_connstr({**defdb, "password": "pwd", "port": "12"})
        == "dbname=db user=uname host=h port=12 password=pwd"
    )
    assert (
        make_connstr({**defdb, "port": "12"}) == "dbname=db user=uname host=h port=12"
    )
...
def test_db_config_to_dict_getuname(monkeypatch):
    ...
    monkeypatch.setattr(
        "LNCDutils.PasswordDialog.user_pass", lambda *x: {"user": "a", "pass": "b"}
    )
    assert (
        make_connstr(db_config_to_dict(defdb, True))
