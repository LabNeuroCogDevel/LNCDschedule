...
def test_sqla_tblins(create_db):
    ...
    tbl = sqla.Table(
        "contact", lsql.sqlmeta, autoload=True, autoload_with=lsql.conn.connection
    )
    fake_contact = {
        "pid": 1,
        "cid": 5,
...
def test_lsql_tblinsert(create_db):
    ...
    nowhere = create_db.connection.execute(
        "select cvalue from contact where cid = 6"
    ).scalar()
...
def test_lsql_update(create_db):
    ...
    lsql.update(
        ...
        new_column="cvalue",
        id_value=7,
        new_value="everywhere",
        id_column="cid",
    )
    assert (
        create_db.connection.execute(
            "select cvalue from contact where cid = 7"
        ).scalar()
        == "everywhere"
    )
...
def test_lsql_update_dict(create_db):
    ...
    assert (
        create_db.connection.execute("select ctype from contact where cid = 7").scalar()
        == "phone"
    )
