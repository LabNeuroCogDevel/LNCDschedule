...
def test_add_ra_insertion(lncdapp, qtbot):
    ...
    ra_assigned = lncdapp.pgtest.connection.execute(
        "select * from ra where ra like 'RA Test 1'"
    ).fetchall()
...
def test_add_study_insertion(lncdapp, qtbot):
    ...
    study_assigned = lncdapp.pgtest.connection.execute(
        "select * from study where study like 'Test Study'"
    ).fetchall()
...
def test_add_task_insertion(lncdapp, qtbot):
    ...
    task_assigned = lncdapp.pgtest.connection.execute(
        "select * from task where task like 'Test Task'"
    ).fetchall()
...
def test_add_visit_type_insertion(lncdapp, qtbot):
    ...
    visit_type_assigned = lncdapp.pgtest.connection.execute(
        "select * from visit where vid = 1"
    ).fetchall()
...
def test_add_multiple_visit_type_insertion(lncdapp, qtbot):
    ...
    visit_type_assigned = lncdapp.pgtest.connection.execute(
        "select * from visit where vid like '1'"
    ).fetchall()
