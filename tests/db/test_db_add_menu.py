...
def test_add_study_insertion(lncdapp, qtbot):
    ...
    assert study_assigned[0][3] == win.study_asigned['visit_types']
...
def test_add_visit_type_insertion(lncdapp, qtbot):
    ...
    win.visit_type_data['vtype'] = "['Test Vtype 1', 'Test Vtype 2']"
    ...
    visit_type_assigned = lncdapp.pgtest.connection.execute("select * from visit where vid like '1'").fetchall()

    assert len(visit_type_assigned) == 1
    assert visit_type_assigned[0][0] == win.visit_type_data['vid']
    assert visit_type_assigned[0][1] == win.visit_type_data['pid']
