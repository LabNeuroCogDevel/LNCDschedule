...
class CheckinVisitWindow(QtWidgets.QDialog):
    ...
    def all_task_disp(self):
        ...
        data = sorted(
            ...
            key=lambda x: (
                ...
                self.vtype
                not in x[2].replace("Questionnaire", "Behavioral").split(" "),
                x[0],
            ),
            reverse=False,
        )
    ...
    def allvals(self, key="all"):
        ...
        if isOrAll(key, "tasks"):
            self.model["tasks"] = [
                self.tasks_list.item(i).text() for i in range(self.tasks_list.count())
            ]
    ...
    def isvalid(self):
        ...
        for k in self.model.keys():
            ...
            if self.model[k] is None or self.model[k] == "" or self.model[k] == []:
    ...
    def checkin_to_db(self, sql):
        ...
        sql.insert(
            "visit_action",
            {"vid": self.model["vid"], "ra": self.model["ra"], "action": "checkedin"},
        )
        ...
        for t in tasks:
            sql.insert(
                "visit_task", {"vid": self.model["vid"], "task": t, "measures": None}
            )
...
if __name__ == "__main__":
    ...
    try:
        ...
        study_tasks = [
            x[0] for x in sql.query.list_tasks_of_study_vtype(study=study, vtype=vtype)
        ]
